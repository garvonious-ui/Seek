...
- Translation picker in ProfileView Settings section (Picker with BibleTranslation.allCases)
- ChatView passes user's `preferredTranslation` to API on every chat request
- Edge Function redeployed with translation support

### Bug fixes confirmed from prior sessions
- Profile auto-create for Apple Sign In — working (ensureRemoteProfile called)
- daily_verses table — seeded with 365 verses in seed.sql
...
### Decisions
- NLT text generated by Claude from training data (not bundled JSON) — no licensing needed
- Daily verses remain KJV-only for now — fast-follow to add NLT versions
- No onboarding change for translation — Settings only
