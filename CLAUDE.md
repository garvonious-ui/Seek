...
- **`.onTapGesture` on a SwiftUI parent steals taps from wrapped UIKit buttons via `UIViewRepresentable`.** A SwiftUI `.onTapGesture` (or `.contentShape(Rectangle()) + .onTapGesture` for tap-outside-to-dismiss-keyboard) on a parent view will consume taps before they reach a UIViewRepresentable-wrapped UIControl underneath, even when the UIKit button has a `addTarget(...)` handler attached. SwiftUI's gesture system wins. Caught Session 17 — added an outer `.onTapGesture { focusedField = nil }` for keyboard dismiss on SignInView, and the `AppleSignInButton` (wrapping `ASAuthorizationAppleIDButton`) became unresponsive — taps registered nowhere. Fix: drop the outer `.onTapGesture`, rely on the keyboard accessory Done button + per-field `.onSubmit` for keyboard dismiss. If you really need tap-outside-to-dismiss next to a wrapped UIControl, use `.simultaneousGesture(TapGesture().onEnded { ... })` instead, which doesn't consume.
- **supabase-swift's `.signedOut` event is not authoritative AND `.signedIn` is not reliable in sheet-over-sheet contexts on iOS 26.4.** Two related rules: (1) Always cross-check `client.auth.currentSession` before treating `.signedOut` as a real sign-out — the SDK can emit spurious `.signedOut` after a successful sign-in during the foreground transition after a modal dismisses. (2) NEVER rely on the `.signedIn` listener event to clear your own derived flags (like `hasOptedForGuest`, `hasCompletedOnboarding`). Set those directly inside the same `MainActor.run` block as the `authState` flip, in every auth-success path (Apple, email sign-in, email sign-up). And keep listener events out of explicit-reset paths — `signOut()` / `deleteAccount()` should set their own state, not rely on the listener side-channel. Build 8 was rejected by App Review for exactly this — the auth listener was supposed to clear `hasOptedForGuest` on `.signedIn`, but in the guest profile → sign-in inline flow on iOS 26.4, a spurious `.signedOut` reverted `authState` while `hasOptedForGuest` was still stuck at `true`, dumping the user back at the guest profile sheet. Caught Session 17 after reviewer attached a screenshot.
